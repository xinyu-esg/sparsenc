...
int main(int argc, char *argv[])
{
    ...
    struct slnc_dec_context_BD *dec_ctx = malloc(sizeof(struct slnc_dec_context_BD));
    slnc_create_dec_context_BD(dec_ctx, sc->meta.datasize, sc->meta.size_b, sc->meta.size_g, sc->meta.size_p, sc->meta.type);
    while (!dec_ctx->finished) {
        struct slnc_packet *pkt = slnc_generate_packet(sc);
        slnc_process_packet_BD(dec_ctx, pkt);
    }
    ...
    slnc_free_dec_context_BD(dec_ctx);
    return 0;
}
